...
async def amain():
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument('--cache-directory',type=str, required=True, help='directory to store selenium and scraper cache files')
    subparsers = parser.add_subparsers(help='modes or operation. Use <mode> --help for more info', dest='mode')

    # create the parser for the 'login' command
    subparsers.add_parser('login', help='initial login to x.com website')

    # create the parser for the 'scrape' command
    parser_scrape = subparsers.add_parser('scrape', help='scrape user mediafiles. Should be used after successful login')
    parser_scrape.add_argument('--user',type=str, required=True, help='twitter user id')
    parser_scrape.add_argument('--output-directory',type=str, required=True, help='directory to save mediafiles')
    ...
    match args.mode:
        case 'login':
            print()
            print('        Selenium window will now open.')
            print('        Log in to website and then return to the terminal.')
            print('        Do not close the window.')
            print()
            driver = s.create_web_driver(headless=False)
            driver.get('https://x.com')
            input('Logged in? press Enter')
            print('Program will now exit. Proceed with scraping as the next step.')
            driver.quit()
